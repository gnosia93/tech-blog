## 카펜터 가중치 노드풀로 비용,성능,ICE 동시에 잡기 — GPU Spot 실전 사례 ##
* 제목 후보..
```
Karpenter 가중치 노드풀: 비용·성능·ICE 한 번에
카펜터 가중치 노드풀로 GPU Spot 최적화하기
가중치 노드풀로 안정적인 GPU Spot 운영하기
```

* "ICE(Insufficient Capacity Error, 용량 부족 에러)"
* g4dn -> g5 ~ 바꾸면 대당 단가는 올라가나 쓰루풋이 좋기 때문에 비용이 비슷하다거나 더 싸질 수 있고, 배치 작업의 시가도 줄어들것으로 기대된다.
