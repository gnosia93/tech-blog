...
옵션 A: LM Studio (초보 추천, GUI)
1. lmstudio.ai에서 앱 다운로드
2. 검색창에 gemma-4-26B 검색
3. Q6_K 버전 다운로드 → Load → 채팅
- 설정 없이 클릭만으로 됨. Mac Metal 가속 자동 적용.

옵션 B: Ollama (터미널, 간단)
brew install ollama
ollama run gemma3   # 사용 가능한 태그는 ollama 라이브러리에서 확인
- Ollama 라이브러리에 해당 모델/양자화가 올라와 있어야 해요.

옵션 C: llama.cpp (직접, 세밀한 제어)
brew install llama.cpp
# Hugging Face에서 Q6_K .gguf 파일을 받은 뒤
llama-cli -m gemma-4-26B-A4B-it-Q6_K.gguf -p "안녕"
```
